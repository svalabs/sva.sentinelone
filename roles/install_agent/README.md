...
| `custom_client_url` | | | Optional URL to a package source to download the SentinelOne client package. If not set, SentinelOne client will be downloaded by via SentinelOne Console. |
| `registration_token` | | | Optional registration token to manually register the agent. Usefull if no API access is available but a registration token was provided by an administrator. If not set the registration token will be gathered from the API. |
